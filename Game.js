class Game {
  ...
  pagoAleatorio(num1, num2) {
    ...
    this.pago =
      Math.floor(
        Math.random() * (num1ToN * (num2ToN / 100) + num1ToN - num1ToN + 1)
      ) + num1ToN;
  }
  displayJuego() {
    if (isNaN(this.price) || isNaN(this.pago)) {
      generadorOptionsP.textContent =
        'Los campos de precio y pago son obligatorios';
  ...
  resultado() {
    ...
    if (parseFloat(vuelto.value) === this.vueltoCorrecto) {
      ...
      paraEnd.textContent =
        'Se acabó el tiempo, ' + 'el vuelto es de: ' + vueltoDeVerdad;
    } else {
      paraEnd.textContent =
        'Incorrecto, el vuelto es de ' +
        vueltoDeVerdad +
        ', tu respuesta fue: ' +
        vuelto.value.toString();
    }
  }
  displayOptions() {
